...
if __name__ == '__main__':
    ...
    data_list = ['./lifted_sampling_fo2/data1/color1_domain10_TV.xlsx']
    domain = 10
    if args.n_samples >= 1000:
        sample_num = args.n_samples // 1000
        sample_str = str(sample_num) + 'k'
    else:
        sample_num = int(args.n_samples)
        sample_str = str(sample_num)
    num = {
        'domain': [],
        'tv': [],
        'real_tv': [],
    ...
    # Note that the domain and weights of the mln or wfomcs files need to be initialized to 0
    last_weight = '0'
    last_domain = '0'
    for d in range(1):
        file_path = data_list[d]
        df = pd.read_excel(file_path)
        # df = df[(df.iloc[:, 0] > 1) & (df.iloc[:, 1] > 1)]   
        for tv in np.arange(0, 1.1, 0.1):
            ...
            for i in range(2):
                ...
                lines[5] = lines[5].replace(last_domain, str(domain + d * 5), 1)
                with open(args.input, 'w', encoding='utf-8') as file:
                    file.writelines(lines)
                last_weight = str(weight[i])
                last_domain = str(domain + d * 5)
                ...
                nums.loc[len(nums)] = [int(lines[5].strip().split()[-1]), tv, df.loc[closest_row, 'TV'],
                                       float(lines[6].strip().split()[0]), closest_row]
                save_file = os.path.join(args.output_dir,
                                         'color1_' + 'domain' + str(domain + d * 5) + '_tv' + str(tv) + '_' + str(
                                             i) + '.pkl')
                ss = []
    ...
    excel_file_path = args.output_dir + '/color1_domain' + str(domain) + '.xlsx'  # 指定输出文件路径
